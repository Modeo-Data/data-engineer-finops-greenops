...
usage_data AS (
    SELECT
        *,
        CAST(DATETIME(usage_start_time, "America/Los_Angeles") AS DATE)
            AS usage_date
    -- *****************************************************************
    ...
    -- *****************************************************************
    AND CAST(DATETIME(usage_start_time, "America/Los_Angeles") AS DATE)
    >= "2019-01-01"
),
...
prices AS (
    SELECT
        ...
        IF(SUM(usage.amount) = 0, 0, SUM(cost) / SUM(usage.amount))
            AS unit_price
    ...
    FROM
        (
            SELECT
                ...
                COALESCE(system_labels.value, "UNKNOWN") AS machine_spec,
                usage.unit,
                prices.unit_price,
                cred.name,
                IF(
                    prices.unit_price = 0
                    OR LOWER(cred.name) NOT LIKE "committed%",
                    0,
                    CASE
                        -- Divide credit $ amount by unit price to calculate amount of usage offset by credit
                        WHEN
                            LOWER(usage.unit) LIKE "seconds"
                            THEN -1 * SUM(cred.amount) / prices.unit_price
                        WHEN
                            LOWER(usage.unit) = "byte-seconds"
                            THEN -1 * SUM(cred.amount) / prices.unit_price
                    END
                ) AS cud_covered_usage,
                IF(LOWER(cred.name) LIKE "committed%", SUM(cred.amount), 0)
                    AS cud_cost,
                IF(
                    prices.unit_price = 0
                    OR LOWER(cred.name) NOT LIKE "sustained%",
                    0,
                    CASE
                        -- Divide credit $ amount by unit price to calculate amount of usage offset by credit
                        WHEN
                ...
                IF(LOWER(cred.name) LIKE "sustained%", SUM(cred.amount), 0)
                    AS sud_cost
...
SELECT
    ...
    SUM(sud_cost) AS sud_cost
FROM cost_data AS a
FULL OUTER JOIN credit_data AS b
    ON
